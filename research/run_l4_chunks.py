...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run checked L4 main dataset chunks.")
    parser.add_argument("--start", type=int, required=True, help="First chunk number, for example 7.")
    parser.add_argument("--end", type=int, required=True, help="Last chunk number, inclusive.")
    parser.add_argument("--chunk-size", type=int, default=500)
    parser.add_argument("--seed-base", type=int, default=4400)
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "train_set" / "L4")
...
def main() -> None:
    ...
    log_file = args.log_file or args.output_dir / f"l4_auto_{args.start:03d}_{args.end:03d}.log"
    ...
    logger.write(f"Started L4 auto run at {datetime.now().isoformat(timespec='seconds')}")
    ...
    for chunk in range(args.start, args.end + 1):
        ...
        output = args.output_dir / f"l4_main_{chunk:03d}_debug.jsonl"
        logger.write("")
        logger.write(f"=== L4 chunk {chunk:03d} seed={seed} output={output} ===")

        run_command(
            [
                sys.executable,
                "research/generate_l4.py",
                "--total",
                str(args.chunk_size),
                "--seed",
                str(seed),
        ...
        run_command([sys.executable, "research/validators/l4.py", str(output)], env, logger)
        ...
        fingerprint_report = fingerprint_l4_chunks(args.output_dir)
        ...
        if fingerprint_report["duplicate_groups"]:
            ...
            raise RuntimeError("Duplicate L4 chunk fingerprint detected")

        logger.write(f"Chunk {chunk:03d} passed all checks.")

    logger.write("")
    logger.write(f"Completed L4 auto run at {datetime.now().isoformat(timespec='seconds')}")
        ...
        while process.poll() is None:
            ...
            if expected_lines is not None and lines >= expected_lines:
                if completed_at is None:
                    ...
                    logger.write(
                        "Process did not exit after expected lines; sending SIGINT and continuing to validation."
                    )
...
def fingerprint_payload(row: dict) -> tuple[object, ...]:
    ...
    return (meta.get("category"), tuple(meta.get("answer_ids") or []), answer)


def fingerprint_l4_chunks(output_dir: Path) -> dict[str, object]:
    groups: dict[tuple[int, str], list[str]] = {}
    chunks: list[tuple[str, int, str]] = []
    total_rows = 0

    for path in sorted(output_dir.glob("l4_main_*_debug.jsonl")):
