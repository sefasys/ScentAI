...
from research.generators.l3 import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate ScentAI L3 dataset records.")
    parser.add_argument("--clean-file", type=Path, default=PROJECT_ROOT / "perfumes_clean.jsonl")
    parser.add_argument("--output", type=Path, default=PROJECT_ROOT / "training_L3_v2.jsonl")
    parser.add_argument("--total", type=int, default=100)
    parser.add_argument("--rag-ratio", type=float, default=0.70)
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "casual_vibe=20,occasion=10,likes_dislikes=15"
        ),
    )
...
def main() -> None:
    ...
    total_records, counts, rag_count = generate_l3_streaming(
        perfumes,
        config,
        query_generator,
    ...
    print("L3 dataset generation complete")
...
def generate_l3_streaming(
    ...
    from research.core.data import build_id_index
    ...
    with output.open(mode, encoding="utf-8") as handle:
        for idx, category in enumerate(schedule[existing:], existing + 1):
            record_rng = random.Random(record_seed(config.seed, idx))
            record = make_l3_record(category, perfumes, indexes, id_index, config, record_rng, query_generator)
