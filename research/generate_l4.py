...
from research.generators.l4 import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate ScentAI L4 grounded reasoning dataset records.")
    parser.add_argument("--clean-file", type=Path, default=PROJECT_ROOT / "perfumes_clean.jsonl")
    parser.add_argument("--output", type=Path, default=PROJECT_ROOT / "training_L4_v2.jsonl")
    parser.add_argument("--total", type=int, default=100)
    parser.add_argument("--rag-ratio", type=float, default=0.88)
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "persona_upgrade=20,occasion_with_tradeoffs=20,style_translation=15"
        ),
    )
...
def main() -> None:
    ...
    total_records, counts, rag_count = generate_l4_streaming(
        perfumes,
        config,
        query_generator,
    ...
    print("L4 dataset generation complete")
...
def generate_l4_streaming(
    ...
    with output.open(mode, encoding="utf-8") as handle:
        for idx, category in enumerate(schedule[existing:], existing + 1):
            ...
            record = make_l4_record(category, perfumes, indexes, config, record_rng, query_generator)
